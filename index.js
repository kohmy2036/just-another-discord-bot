...
bot.on("message", async function (message) {
  ...
  async function geturl(msg) {
    var fetchedMsg = await message.channel.fetchMessage(msg);
    var mUrl = await fetchedMsg.url;
    return mUrl;
  }

  switch (command) {
    default: {console.log(command); break;} //eslint-disable-line
    ...
    case "say": {
      let sayThis = message.channel.lastMessage.toString().replace(PREFIX + command, "");
    ...
    case "user": {
      ...
      let muted = function () { if (!member.mute) return "No"; else return "Yes"; };

      //.replace(/T/, ' ')      // replace T with a space
      //.replace(/\..+/, '');     // delete the dot and everything after

      message.channel.send({
        embed: { //bloody hell embeds
          author: {
